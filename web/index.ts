...
const App = (_: any, state: AppState, setState: SetState) => {
  const setLoadingState = (newState: Partial<AppState>) => {
    ...
    if (state.overrideUrl && state.overrideUrl !== reNewState.overrideUrl) {
      reNewState.overrideUrl = state.overrideUrl;
    }
    if (reNewState.overrideUrl) {
      timeout = window.setTimeout(() => setState({ overrideUrl: null }), 200);
    }

    setState({ ...reNewState, loading: true });
  };
  const {
    fileType = 'png',
    fontSize = '100px',
